...
fn item() -> SourceItem {
    source_downloader_sdk::serde_json::from_value(source_downloader_sdk::serde_json::json!({"title":"Show","link":"https://example.test/show","datetime":"2025-01-01T00:00:00Z","contentType":"video","downloadUri":"https://example.test/show.mkv"})).unwrap()
}
fn benchmark_resolution_variable_provider(criterion: &mut Criterion) {
    let runtime = tokio::runtime::Builder::new_current_thread().build().unwrap();
    let provider = component();
    let item = item();
    let item_variables = HashMap::new();
    let mut group = criterion.benchmark_group("resolution_variable_provider");
    for count in [1, 16, 256] {
        let files: Vec<_> = (0..count)
            .map(|index| {
                SourceFile::new(PathBuf::from(format!("Show-{index}-3840x2160.mkv")))
            })
            .collect();
...
criterion_group!(benches, benchmark_resolution_variable_provider);

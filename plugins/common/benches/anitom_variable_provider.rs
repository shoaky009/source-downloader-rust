...
fn benchmark_anitom_variable_provider(criterion: &mut Criterion) {
    let runtime = tokio::runtime::Builder::new_current_thread().build().unwrap();
    let provider = component();
    let item = item();
    let item_variables = HashMap::new();
    let mut group = criterion.benchmark_group("anitom_variable_provider");
    for count in [1, 16, 256] {
        let files: Vec<_> = (0..count)
            .map(|index| {
                SourceFile::new(PathBuf::from(format!(
                    "[Group] Show - {index:03}v2 [1920x1080 H.264 FLAC][1234ABCD].mkv"
                )))
            })
            .collect();
...
criterion_group!(benches, benchmark_anitom_variable_provider);

...
fn component() -> Arc<dyn VariableProvider> {
    let supplier = PLUGIN
        ...
        .unwrap();
    let props = Map::from_iter([("read-content".to_owned(), Value::Bool(false))]);
...
fn benchmark_language_variable_provider(criterion: &mut Criterion) {
    let runtime = tokio::runtime::Builder::new_current_thread().build().unwrap();
    let provider = component();
    let item = item();
    let item_variables = HashMap::new();
    let mut group = criterion.benchmark_group("language_variable_provider");
    for count in [1, 16, 256] {
        let files: Vec<_> = (0..count)
            .map(|index| {
                let marker = ["CHS", "CHT", "JPCHS", "ENG", "unmarked"][index % 5];
                SourceFile::new(PathBuf::from(format!(
                    "Show - {index:03} [{marker}].ass"
                )))
            })
            .collect();
...
criterion_group!(benches, benchmark_language_variable_provider);

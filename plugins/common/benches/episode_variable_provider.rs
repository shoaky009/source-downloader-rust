...
fn item() -> SourceItem {
    source_downloader_sdk::serde_json::from_value(source_downloader_sdk::serde_json::json!({
        "title":"Show", "link":"https://example.test/show", "datetime":"2025-01-01T00:00:00Z",
        "contentType":"video", "downloadUri":"https://example.test/show.mkv"
    })).unwrap()
}

fn benchmark_episode_variable_provider(criterion: &mut Criterion) {
    let runtime = tokio::runtime::Builder::new_current_thread().build().unwrap();
    let provider = component();
    let item = item();
    let item_variables = HashMap::new();
    let mut group = criterion.benchmark_group("episode_variable_provider");
    for count in [1, 16, 256] {
        let files: Vec<_> = (1..=count)
            .map(|episode| {
                SourceFile::new(PathBuf::from(format!(
                    "[Group] Show S02E{episode:03} [1920x1080][HEVC10][1234ABCD].mkv"
                )))
            })
            .collect();
...
criterion_group!(benches, benchmark_episode_variable_provider);
